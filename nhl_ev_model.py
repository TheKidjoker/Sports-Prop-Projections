"""
NHL EV Model — L2-regularized logistic regression for NHL spread prediction.

Replaces the heuristic additive scoring system with a calibrated probability
model that feeds an EV-based betting framework:
    Edge = Model_Probability - Implied_Market_Probability

Designed for low variance with 529 games — 8 features, strong L2
regularization, validated via strict walk-forward rolling windows only.

Features (8, respecting tight 529-game budget):
...
Excluded (noise/harmful per backtest evidence):
    - Friday penalty (-3): standalone lift < 2%, narrative-based
    - H2H revenge/dominance: both zeroed, p > 0.15
    - Home/away split: zeroed, marginal ~0%
    - ATS bonus/penalty: zeroed, noise
    - Public slot bonus: absorbed into CLV
    - Slot classification: absorbed into CLV
    - Line direction: r=0.68 overlap with line_movement, removed
    - Win pct features: dropped to reduce dimensionality (529 game budget)

Training: walk-forward rolling only (250-train / 50-test, step 50).
AUC gate: reject if AUC < 0.53.
Edge tiers: STRONG 8%+, CONFIDENT 5%+, LEAN 3%+.
"""
...
NHL_EV_CONFIG = {
    ...
    "warmup_games": 20,
    "rolling_window": 20,
    "regressed_prior_weight": 12,   # Slightly heavier shrinkage for NHL variance
    "league_avg_goals": 3.0,        # NHL league average goals/game
    "default_total": 5.8,           # NHL default game total
    "feature_names": [
        "spread_abs", "clv", "line_movement_abs", "rest_diff",
        "dog_gf_regressed", "fav_gf_regressed", "goal_diff_net", "total",
        "elo_diff",
...
def _regressed_avg(all_scores, recent_n=5):
    ...
    prior_weight = NHL_EV_CONFIG["regressed_prior_weight"]
    default = NHL_EV_CONFIG["league_avg_goals"]
    if not all_scores:
        return default
...
def _days_since_last(team_st, game_date_str):
    """Days since team's last game. Returns 2 if unknown (NHL ~every 2 days)."""
...
def _extract_features(game, team_state):
    """
    Extract feature vector for one NHL game.
    ...
    # Feature 3: line_movement_abs — magnitude only (direction dropped; r=0.68 overlap)
    ...
    # Feature 5-6: regressed goals for (offensive efficiency proxy)
    dog_gf_regressed = _regressed_avg(dog_st["scores"])
    fav_gf_regressed = _regressed_avg(fav_st["scores"])

    # Feature 7: goal_diff_net — (dog_GF - dog_GA) - (fav_GF - fav_GA)
    dog_ga = _regressed_avg(dog_st.get("opp_scores", []))
    fav_ga = _regressed_avg(fav_st.get("opp_scores", []))
    dog_net = dog_gf_regressed - dog_ga
    fav_net = fav_gf_regressed - fav_ga
    goal_diff_net = dog_net - fav_net
    ...
    return {
        ...
        "rest_diff": rest_diff,
        "dog_gf_regressed": dog_gf_regressed,
...
def build_nhl_ev_dataset():
    """
    Build chronological dataset from tm_historical_games for NHL.
    ...
    games = tm_db.get_historical_games("nhl")
    if not games:
        return []
    ...
    warmup = NHL_EV_CONFIG["warmup_games"]
    window = NHL_EV_CONFIG["rolling_window"]
...
def train_nhl_ev_model(dataset):
    """
    Train via strict walk-forward rolling validation.
    No single split — the user requires walk-forward only.
    ...
    feature_names = NHL_EV_CONFIG["feature_names"]
    C = NHL_EV_CONFIG["regularization_C"]
    train_size = NHL_EV_CONFIG["rolling_train_size"]
    test_size = NHL_EV_CONFIG["rolling_test_size"]
    step = NHL_EV_CONFIG["rolling_step"]
    ...
    implied = NHL_EV_CONFIG["implied_prob"]
    ...
    # AUC gate
    auc_gate = NHL_EV_CONFIG["auc_gate"]
...
def get_ev_recommendation(model_prob):
    ...
    edge = model_prob - NHL_EV_CONFIG["implied_prob"]
    tiers = NHL_EV_CONFIG["edge_tiers"]
...
def _load_model_to_cache(result):
    ...
    print(f"[nhl_ev] Model loaded — AUC: {result.get('mean_auc')}, "
          f"valid: {result['is_valid']}, monotonic: {result['monotonicity_pass']}",
          flush=True)


def load_nhl_ev_model(model_params):
    """
    Load a previously saved NHL EV model from model_params dict.
    Called at app startup from tm_model_runs.
    """
    global _nhl_ev_cache

    auc = model_params.get("mean_auc") or model_params.get("auc", 0)
    is_valid = auc >= NHL_EV_CONFIG["auc_gate"]

    _nhl_ev_cache = {
        "model": None,
        "scaler_mean": model_params.get("scaler_mean", []),
        "scaler_std": model_params.get("scaler_std", []),
        "feature_names": model_params.get("feature_names", NHL_EV_CONFIG["feature_names"]),
        "auc": auc,
        "is_valid": is_valid,
    ...
    if is_valid:
        print(f"[nhl_ev] Model restored from DB — AUC: {auc}", flush=True)
    else:
        print(f"[nhl_ev] Model loaded but below AUC gate ({auc} < {NHL_EV_CONFIG['auc_gate']})", flush=True)


def is_ev_model_active():
    """Check if NHL EV model is loaded and valid."""
    return _nhl_ev_cache.get("is_valid", False)


def predict_single(features_dict):
    """
    Predict underdog cover probability for a single NHL game.
...
def load_live_team_state():
    ...
    try:
        ...
        print(f"[nhl_ev] Live team state loaded: {len(team_state)} teams", flush=True)
...
def run_full_training():
    ...
    with _ev_lock:
        _ev_progress.update({
            ...
            "message": "Building NHL feature dataset from historical games...",
        })

    dataset = build_nhl_ev_dataset()
    if not dataset:
        with _ev_lock:
            _ev_progress.update({
                "status": "error",
                "message": "No dataset could be built. Collect more NHL historical data.",
            })
        return
    ...
    result = train_nhl_ev_model(dataset)
    if "error" in result:
        with _ev_lock:
            _ev_progress.update({
                "status": "error",
...
def start_ev_training_thread():
    """Start NHL EV training in a background thread."""
    with _ev_lock:
